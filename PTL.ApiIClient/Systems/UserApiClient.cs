...
namespace PTL.ApiIClient
{
    public class UserApiClient : IUserApiClient
    {
        ...
        public async Task<PagedResult<UserVm>> GetSelectAll(GetPagingRequest request)
        {
            var client = _httpClientFactory.CreateClient();
            var sessions = _httpContextAccessor.HttpContext.Session.GetString("Token");
            ...
            client.BaseAddress = new Uri(_configuration["BaseAddress"]);
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", sessions);
            var response = await client.GetAsync($"/api/users?pageIndex=" +
                $"{request.PageIndex}&pageSize={request.PageSize}&keyword={request.Keyword}");
            var body = await response.Content.ReadAsStringAsync();
    ...
    public interface IUserApiClient
    {
        ...
        Task<PagedResult<UserVm>> GetSelectAll(GetPagingRequest request);
    }
}