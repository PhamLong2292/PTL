...
function open_from_Image_Init(img, callback, canceled) {
    are_you_sure(function () {
        // TODO: shouldn't open_from_* start a new session?
...
function open_from_FileWithUrl(sourceUrl, file, callback, canceled) {
    sourceURL = sourceUrl;
    var blob_url = URL.createObjectURL(file);
    load_image_from_URI(blob_url, function (err, img) {
        // revoke object URL regardless of error
	...
	if(window.chrome && chrome.fileSystem && chrome.fileSystem.chooseEntry){
		...
		canvas.toBlob(function(blob){
			sanity_check_blob(blob, function(){
			    ...
			    // "       }\r\n" +
                   data.append("ReplaceFile", blob,file_name);
			    //       Make Ajax request with the contentType = false, and procesDate = false
			           var ajaxRequest = $.ajax({
                           type: 'POST',
                           url: window.location.origin + "/services/ReplaceFileService.asmx/ReplaceFile",
                           contentType: false,
                           processData: false,
                           data: data,
